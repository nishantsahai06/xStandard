"""Fault header builder — constructs the S1000D DM header deterministically.

Every header field is derived via **DIRECT** copy or **RULE**-based
generation.  No LLM involvement — this service is fully auditable
and reproducible.

Responsibility: build ``FaultHeader`` (dmCode, language, issueInfo,
issueDate, dmTitle) from source metadata and the resolved fault mode.
"""

from __future__ import annotations

from fault_mapper.domain.enums import FaultMode, MappingStrategy
from fault_mapper.domain.models import DocumentPipelineOutput, FaultHeader
from fault_mapper.domain.ports import RulesEnginePort
from fault_mapper.domain.value_objects import FieldOrigin


class FaultHeaderBuilder:
    """Builds the S1000D data-module header from source metadata.
    ...
    def __init__(self, rules: RulesEnginePort) -> None:
        self._rules = rules
    ...
    def build(
        ...
        mode: FaultMode,
    ) -> tuple[FaultHeader, dict[str, FieldOrigin]]:
        ...
        dm_code = self._rules.build_dm_code(source, mode)
        ...
        dm_title = self._rules.normalize_title(raw_title, mode)
